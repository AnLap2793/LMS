...
- **Video Lesson:**
  - External: Embed YouTube/Vimeo/Loom (chỉ cần paste URL)
  - Internal: Upload video trực tiếp lên Directus Files (cho nội dung bảo mật)
...
*(Phần này sẽ được bổ sung chi tiết trong tương lai)*
...
| type | string | Yes | - | Loại bài học: "video", "article", "file", "link", "scorm" | select-dropdown | labels |
| content | text (markdown) | No | null | Nội dung bài học (nếu type = "article") | markdown | formatted-text |
| video_url | string | No | null | URL video (YouTube/Vimeo URL) | input | raw |
| video_provider | string | No | null | Provider: "youtube", "vimeo", "direct" | select-dropdown | labels |
| video_id | string | No | null | Video ID (extracted từ URL) | input | raw |
